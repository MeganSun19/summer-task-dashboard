...
def load_workbook_records(path: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    with ZipFile(path) as archive:
        strings = shared_strings(archive)
        targets = sheet_targets(archive)
        for required in ["课程阶段", "课程模块"]:
...
def main() -> int:
    ...
    try:
        if args.model:
            ...
            stage_records, module_records = load_workbook_records(args.input)
            source = {
                "type": "xlsx",
                "file": str(Path("..") / args.input.relative_to(PROJECT_ROOT.parent)),
                "sha256": hashlib.sha256(args.input.read_bytes()).hexdigest(),
            }
        generated = render_json(build_catalog(stage_records, module_records, source))
        if args.check:
            if not args.output.exists() or args.output.read_text(encoding="utf-8") != generated:
                print("课程计划生成结果已过期。请运行：python3 scripts/generate-course-plan-catalog.py", file=sys.stderr)
                return 1
